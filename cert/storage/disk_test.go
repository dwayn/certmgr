...
func getFilesAndPaths(ca, cert, key string) (caFile *file.CertificateFile, certFile *file.CertificateFile, keyFile *file.File, paths []string) {
	if ca != "" {
		caFile = &file.CertificateFile{File: file.File{Path: ca}}
		paths = append(paths, ca)
	}
	if cert != "" {
		certFile = &file.CertificateFile{File: file.File{Path: cert}}
		paths = append(paths, cert)
	}
	if key != "" {
		keyFile = &file.File{Path: key}
